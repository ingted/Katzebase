    ...
    internal class TabFilePage : TabPage, IDisposable
    {
        ...
        private void ExecuteCurrentScriptSync(KbClient client, string scriptText, ExecuteType executeType)
        {
            ...
            try
            {
                ...
                {
                    var results = client.Query.ExplainPlans(scripts, Program.Settings.QueryTimeOut >= 0 ? TimeSpan.FromSeconds(Program.Settings.QueryTimeOut) : Timeout.InfiniteTimeSpan);
                ...
                {
                    var results = client.Query.ExplainOperations(scripts, Program.Settings.QueryTimeOut >= 0 ? TimeSpan.FromSeconds(Program.Settings.QueryTimeOut) : Timeout.InfiniteTimeSpan);
                ...
                {
                    var results = client.Query.FetchMultiple(scripts, Program.Settings.QueryTimeOut >= 0 ? TimeSpan.FromSeconds(Program.Settings.QueryTimeOut) : Timeout.InfiniteTimeSpan);
